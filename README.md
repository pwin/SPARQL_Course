...
A teaching dataset in RDF, and 145 worked queries that take a beginner from
...
At 145 queries, reading front to back is not the only way in.
...
All three engines can infer new triples. No two of them do the same thing, and
the differences are large enough to change what your queries return. Measured
on the same 4,826 triples:

| | distinct triples | usefully new | `bs:within` |
|---|---|---|---|
| asserted, no reasoning | 4,826 | — | 63 |
| Jena `riotcmd.infer --rdfs` | 4,826 | 0 | 63 |
| HOLOS `holos entail` | 5,051 | 0 | 63 |
| HyLAR OWL 2 RL — the editor's **Show Facts** | 8,777 | 3,952 | **186** |
| Jena OWLMicro, via an assembler description | — | — | **186** |
| `?s bs:within+ ?o` — no reasoner at all | 4,826 | 0 | **186** |

Two things fall out of that.

**Neither RDFS reasoner derives a usable new fact here.** That is not a
failing: the class hierarchy is six subclass pairs deep and every instance is
already typed, so the rule that does most of RDFS's work has nothing left to
do. The raw counts flatter both — Jena emits 9,375 lines that reduce to 4,826
distinct triples, all of them already asserted; HOLOS writes 225, of which 133
are reflexive axioms and the other 92 type things as a class with **no name**,
the anonymous `owl:unionOf` that two properties declare as their domain. True,
and not something you can query for.

**A property path gets the OWL answer for free.** `bs:within` is declared
`owl:TransitiveProperty`; RDFS has no transitivity rule, so both RDFS
reasoners leave the closure at 63. The two OWL reasoners close it to 186 — and
so does one line of SPARQL 1.1, on every engine, with nothing switched on and
nothing stored. A reasoner writes those 186 pairs down and they go stale the
moment a place moves; the path recomputes them and is never wrong.

[Module 18](queries/18-inference/) writes those inferences as queries. The
course document's **Reasoning in the three engines** section has the
invocations for each engine, HOLOS's entailed-graph design, Jena's assembler
route to the OWL reasoners, and why none of the three implements the
[entailment regimes](https://www.w3.org/TR/sparql12-entailment/) the
specification actually describes — they all materialise instead.
...
| `queries_core.py`, `queries_paths.py`, `queries_geo.py`, `queries_rdf12.py`, `queries_forms.py`, `queries_debug.py`, `queries_extensions.py`, `queries_federation.py`, `queries_blanknodes.py`, `queries_update.py`, `queries_toolkit.py`, `queries_inference.py` | the 145 queries and their explanations |
...
| `reasoning_section.py` | what HyLAR, `holos entail` and Jena's two reasoners derive from the same 4,826 triples, and the commands that measured it |
